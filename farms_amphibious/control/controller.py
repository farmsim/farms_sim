...
class AmphibiousController(ModelController):
    ...
    def __init__(self, joints, animat_options, animat_data, timestep):
        ...
        self.gain_amplitude = np.array([
            animat_options.control.network.joints.gain_amplitude[joint]
            for joint in joints
        ])
        self.gain_offset = np.array([
            animat_options.control.network.joints.gain_offset[joint]
            for joint in joints
        ])
        self.joints_offsets = np.array([
            animat_options.control.network.joints.offsets[joint]
            for joint in joints
        ])
